// =============================================================================
// Edge Function: reject-repair-request
// =============================================================================
// Role: shop_owner only
// Description: Sets the status of a repair request to 'rejected'.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (userProfile.role !== "shop_owner") {
      return new Response(
        JSON.stringify({ error: "Forbidden: Only shop owners can reject repair requests" }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
